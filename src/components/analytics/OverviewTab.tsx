...
const HeatmapCell = ({ val, name }: { val: number; name: string }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-1 group/cell w-full">
      ...
      >
        <span className={cn('text-[9px] font-black font-syne italic', Math.abs(val) > 5 ? 'text-white' : 'text-white/40')}>
          {val !== 0 ? `${val > 0 ? '+' : ''}${val}%` : ''}
        </span>
      ...
      <span className="text-[7px] font-black text-white/10 uppercase tracking-wider font-syne">{name}</span>
    </div>
  );
};
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#08080c]/95 border-2 border-white/10 rounded-[24px] p-6 shadow-[0_40px_80px_rgba(0,0,0,0.9)] backdrop-blur-3xl">
        <p className="text-[10px] font-black text-p uppercase tracking-[0.4em] font-syne mb-3">{label}</p>
        {payload.map((p: any, i: number) => (
          <div key={i} className="flex items-center gap-3 mt-1">
            <div className="w-2 h-2 rounded-full" style={{ background: p.stroke }} />
            <span className="text-sm font-black text-white font-syne italic">
              ${Number(p.value).toLocaleString()}
              <span className="text-[10px] text-white/20 ml-2 uppercase tracking-widest font-normal">{p.dataKey}</span>
            </span>
          </div>
        ))}
      </div>
            ...
            <Card className="p-[var(--card-p)] border-2 border-white/5 bg-black/40 backdrop-blur-3xl shadow-[0_30px_60px_rgba(0,0,0,0.5)] relative overflow-hidden group hover:border-white/20 transition-all duration-700 rounded-[32px] h-full">
              ...
              <div className="space-y-6 relative z-10">
                <div className="flex items-center justify-between">
                  ...
                  <span className={cn('text-[10px] font-black font-syne italic px-3 py-1 rounded-full border', 
                ...
                <div>
                  <p className="text-[9px] font-black text-white/30 uppercase tracking-[0.3em] font-syne italic">{kpi.label}</p>
                  <h3 className="text-2xl font-black text-white italic tracking-tighter mt-1 font-syne">{kpi.value}</h3>
                  <p className="text-[8px] text-white/10 font-black uppercase tracking-[0.2em] mt-2 font-syne">{kpi.desc}</p>
                </div>
              </div>
            </Card>
          </motion.div>
          ...
          <Card className="p-[var(--card-p)] border-2 border-white/5 bg-black/40 backdrop-blur-3xl shadow-[0_50px_100px_rgba(0,0,0,0.8)] overflow-hidden relative group rounded-[40px] h-full">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-8 mb-8 relative z-10">
              <div className="space-y-3">
                <div className="flex items-center gap-4">
                  ...
                  <h3 className="text-xl font-black text-white uppercase tracking-tighter font-syne italic">Equity_Lattice</h3>
                </div>
                <p className="text-[10px] text-white/20 font-black uppercase tracking-[0.4em] font-syne italic max-w-sm">
              ...
              <div className="flex bg-black/60 p-1.5 rounded-[22px] border border-white/5 backdrop-blur-xl shrink-0">
                 {['1M', '3M', '1Y', 'NEURAL_MAX'].map((p) => (
                   <button
                     ...
                     className={cn(
                       'px-5 py-2 rounded-[18px] text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-500 font-syne italic',
                       activePeriod === p
            ...
            <div className="flex items-center gap-8 mt-4 pl-2 relative z-10">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em] font-syne italic">Master_Equity</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-0.5 bg-cyan-400 rounded-full" style={{ borderTop: '2px dashed #06b6d4', background: 'none' }} />
                <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em] font-syne italic">Variance_Alpha</span>
              </div>
              <div className="ml-auto text-right">
                <p className="text-[8px] text-white/10 font-black uppercase tracking-[0.4em] font-syne italic">Total_P&L</p>
                <p className="text-lg font-black text-emerald-400 font-syne italic tracking-tighter">+$20,400 <span className="text-[10px] text-emerald-400/50 uppercase tracking-widest">+20.4%</span></p>
              </div>
            </div>
          </Card>
        </motion.div>
          ...
          <Card className="p-[var(--card-p)] border-2 border-white/5 bg-black/40 backdrop-blur-3xl shadow-[0_50px_100px_rgba(0,0,0,0.8)] h-full flex flex-col relative overflow-hidden group rounded-[40px]">
            ...
            <div className="mb-6 relative z-10">
              <div className="flex items-center gap-4">
                ...
                <h4 className="text-lg font-black text-white uppercase tracking-tighter font-syne italic">Neural_Yield_Map</h4>
              </div>
              <p className="text-[10px] text-white/20 font-black uppercase tracking-[0.3em] mt-3 leading-loose italic font-syne">
            ...
            <div className="space-y-6 flex-1 relative z-10">
              {heatmapData.map((yearData) => (
                <div key={yearData.year} className="space-y-3">
                  <div className="flex items-center gap-4">
                    <span className="text-[10px] font-black text-white/30 tracking-[0.5em] uppercase font-syne italic">{yearData.year}</span>
            ...
            <div className="mt-6 flex items-center justify-between border-t border-white/5 pt-5 relative z-10">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] font-syne italic">Drawdown</span>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] font-syne italic">Alpha</span>
            ...
            <div className="flex items-start justify-between relative z-10">
              <div className="space-y-4">
                <div className="flex items-center gap-3 px-4 py-2 bg-emerald-500/10 rounded-xl border border-emerald-500/20 w-fit">
                  ...
                  <span className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.4em] font-syne italic">Alpha_Regime_Peak</span>
                </div>
                <h5 className="text-4xl font-black text-white italic tracking-tighter font-syne leading-none">
                  +12.34%
            ...
            <div className="mt-8 pt-8 border-t border-white/5 space-y-5 relative z-10">
              {[
                ...
                <div key={bar.label} className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-black text-white/30 uppercase tracking-[0.3em] font-syne italic">{bar.label}</span>
                    <span className="text-[10px] font-black text-white/50 font-syne italic">{bar.val}%</span>
                  </div>
              ...
              <button className="flex items-center gap-4 text-[11px] font-black text-emerald-400 uppercase tracking-[0.5em] group/btn hover:text-white transition-all font-syne italic mt-4">
        ...
        <motion.div variants={itemVariants}>
          <Card className="p-[var(--card-p)] border-2 border-p/10 bg-p/[0.02] backdrop-blur-3xl relative overflow-hidden group hover:border-p/30 transition-all duration-700 rounded-[40px] shadow-[0_40px_80px_rgba(0,0,0,0.5)]">
            ...
            <div className="flex items-start justify-between relative z-10">
              <div className="space-y-4">
                <div className="flex items-center gap-3 px-4 py-2 bg-p/10 rounded-xl border border-p/20 w-fit">
                  ...
                  <span className="text-[10px] font-black text-p uppercase tracking-[0.4em] font-syne italic">Stress_Lattice_Score</span>
                </div>
                <h5 className="text-4xl font-black text-white italic tracking-tighter font-syne leading-none">
            ...
            <div className="mt-8 pt-8 border-t border-white/5 space-y-5 relative z-10">
              ...
              <button className="flex items-center gap-4 text-[11px] font-black text-p uppercase tracking-[0.5em] group/btn hover:text-white transition-all font-syne italic mt-4">
        ...
        <Card className="p-[var(--card-p)] border-2 border-white/5 bg-black/40 backdrop-blur-3xl shadow-[0_40px_80px_rgba(0,0,0,0.6)] relative overflow-hidden group rounded-[40px]">
          ...
          <div className="flex items-center justify-between mb-8 relative z-10">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h4 className="text-lg font-black text-white uppercase tracking-tighter font-syne italic">Live_Event_Ledger</h4>
                <p className="text-[10px] text-white/20 font-black uppercase tracking-[0.3em] font-syne italic">Real-time signal intelligence</p>
              </div>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.3em] font-syne italic">LIVE</span>
            </div>
          </div>
            ...
            {[
              ...
              >
                <div className="flex items-start justify-between gap-4 mb-4">
                  <span className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] font-syne italic font-mono">{e.time}</span>
                  <span className={cn(
                    'text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full border font-syne italic',
                ...
                <p className="text-sm font-black text-white uppercase tracking-tight font-syne italic leading-tight">{e.event}</p>
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/5">
                  <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em] font-syne italic">{e.asset}</span>
                  <span className={cn('text-sm font-black font-syne italic', e.pnl.startsWith('+') ? 'text-emerald-400' : 'text-red-400')}>{e.pnl}</span>
                </div>
              </motion.div>
            ))}
          </div>
