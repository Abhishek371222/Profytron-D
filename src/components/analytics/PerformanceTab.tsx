  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#08080c]/95 border-2 border-white/10 rounded-[20px] p-5 shadow-[0_30px_60px_rgba(0,0,0,0.9)] backdrop-blur-3xl">
        <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.4em] font-syne mb-3">{label}</p>
        {payload.map((p: any, i: number) => (
          <div key={i} className="flex items-center gap-3 mt-1.5">
            <div className="w-2 h-2 rounded-full" style={{ background: p.fill }} />
            <span className="text-sm font-black text-white font-syne italic capitalize">
            ...
            <Card className="p-[var(--card-p)] border-2 border-white/5 bg-black/40 backdrop-blur-3xl rounded-[32px] relative overflow-hidden group hover:border-p/30 transition-all duration-700 shadow-[0_30px_60px_rgba(0,0,0,0.5)] h-full">
              ...
              <div className="space-y-5 relative z-10">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black text-white/30 uppercase tracking-[0.4em] font-syne italic">{s.name}</span>
                  <div className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)] animate-pulse" />
                </div>
                <div>
                  <h3 className="text-2xl font-black text-white italic tracking-tighter font-syne">{s.winRate}%</h3>
                  <p className="text-[8px] text-white/20 font-black uppercase tracking-[0.2em] font-syne italic mt-1">Win_Rate</p>
                </div>
                <div className="space-y-2 pt-3 border-t border-white/5">
                  <div className="flex justify-between text-[10px]">
                    <span className="text-white/20 font-black font-syne italic uppercase tracking-wider">Return_30d</span>
                    <span className="text-emerald-400 font-black font-syne italic">+{s.return30d}%</span>
                  </div>
                  <div className="flex justify-between text-[10px]">
                    <span className="text-white/20 font-black font-syne italic uppercase tracking-wider">Sharpe_K</span>
                    <span className="text-cyan-400 font-black font-syne italic">{s.sharpe}</span>
                  </div>
                  <div className="flex justify-between text-[10px]">
                    <span className="text-white/20 font-black font-syne italic uppercase tracking-wider">Trades</span>
                    <span className="text-white/60 font-black font-syne italic">{s.trades}</span>
                  </div>
                </div>
              </div>
            </Card>
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-8 mb-8 relative z-10">
            <div className="space-y-3">
              <div className="flex items-center gap-4">
                ...
                <h3 className="text-xl font-black text-white uppercase tracking-tighter font-syne italic">Strategy_Benchmark</h3>
              </div>
              <p className="text-[10px] text-white/20 font-black uppercase tracking-[0.4em] font-syne italic max-w-lg">
          ...
          <Card className="p-[var(--card-p)] border-2 border-white/5 bg-black/40 backdrop-blur-3xl shadow-[0_50px_100px_rgba(0,0,0,0.8)] relative overflow-hidden group h-full rounded-[40px]">
            ...
            <div className="mb-8">
              <div className="flex items-center gap-4">
                ...
                <h4 className="text-lg font-black text-white uppercase tracking-tighter font-syne italic">Correlation_Lattice</h4>
              </div>
              <p className="text-[10px] text-white/20 font-black uppercase tracking-[0.3em] mt-3 italic font-syne">
              ...
              <div className="grid grid-cols-5 gap-3 pl-20">
                {strategyNames.map(n => (
                  <div key={n} className="flex items-end justify-center h-16">
                    <span className="text-[8px] font-black text-white/20 uppercase font-syne italic" style={{ writingMode: 'vertical-rl', transform: 'rotate(180deg)' }}>{n}</span>
                  </div>
                ))}
              </div>
              {correlationMatrix.map((row, i) => (
                <div key={i} className="grid grid-cols-5 gap-3 items-center">
                  <span className="text-[9px] font-black text-white/20 uppercase font-syne italic truncate">{strategyNames[i].slice(0, 6)}</span>
                  {row.map((val, j) => {
                    ...
                    return (
                      ...
                      >
                        <span className={cn(
                          'text-[11px] font-black font-syne italic',
          ...
          <Card className="p-[var(--card-p)] border-2 border-white/5 bg-black/40 backdrop-blur-3xl shadow-[0_50px_100px_rgba(0,0,0,0.8)] h-full flex flex-col relative overflow-hidden group rounded-[40px]">
            ...
            <div className="mb-8">
              <div className="flex items-center gap-4">
                ...
                <h4 className="text-lg font-black text-white uppercase tracking-tighter font-syne italic">Temporal_Gate_Density</h4>
              </div>
              <p className="text-[10px] text-white/20 font-black uppercase tracking-[0.3em] mt-3 italic font-syne">
            ...
            <div className="flex flex-col gap-2 flex-1 relative z-10 overflow-hidden">
              {timeHeatmap.map((row, dIdx) => (
                <div key={days[dIdx]} className="flex gap-1.5 items-center">
                  <span className="w-12 text-[9px] font-black text-white/20 uppercase font-syne italic shrink-0">{days[dIdx]}</span>
            ...
            <div className="flex items-center justify-between mt-6 pt-5 border-t border-white/5">
              <span className="text-[9px] font-black text-white/10 uppercase tracking-[0.4em] font-syne italic">00:00</span>
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded bg-emerald-500/70 shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
                  <span className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] font-syne italic">Growth</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded bg-red-500/50" />
                  <span className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] font-syne italic">Risk</span>
                </div>
              </div>
              <span className="text-[9px] font-black text-white/10 uppercase tracking-[0.4em] font-syne italic">23:00</span>
            </div>
          </Card>
        </motion.div>
      </div>
      ...
      <motion.div variants={itemVariants}>
        <div className="p-10 rounded-[40px] border-2 border-p/20 bg-p/[0.04] backdrop-blur-3xl flex flex-col md:flex-row items-center justify-between gap-10 relative overflow-hidden group shadow-[0_60px_120px_rgba(0,0,0,0.5)]">
          ...
          <div className="flex items-center gap-8 relative z-10">
            ...
            <div>
              <h3 className="text-3xl font-black text-white italic tracking-tighter font-syne uppercase">Neural_Recal_v4</h3>
              <p className="text-[11px] text-white/30 font-black mt-3 uppercase tracking-[0.4em] max-w-lg leading-relaxed font-syne italic">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="h-16 px-12 rounded-[24px] bg-p text-white text-[13px] font-black uppercase tracking-[0.5em] shadow-[0_20px_40px_rgba(99,102,241,0.4)] relative z-10 font-syne italic overflow-hidden group/btn shrink-0"
          >
