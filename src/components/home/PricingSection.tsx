          ...
          <FadeUp>
            ...
            <div className="flex items-center justify-center gap-4 mt-12 bg-white/[0.03] border border-white/5 p-1.5 rounded-[22px] w-fit mx-auto backdrop-blur-3xl shadow-2xl relative overflow-hidden group">
              ...
              <button
                ...
                className={cn(
                  "px-8 py-3 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] transition-all duration-500 relative z-10 font-syne",
              ...
              >
                Epoch_Yearly
                <span className="absolute -top-4 -right-1 px-2 py-0.5 bg-emerald-500 text-[8px] text-white rounded-md font-black shadow-lg animate-pulse font-syne">
                  SAVE_20%
                </span>
              </button>
            </div>
              ...
              >
                ...
                >
                  ...
                  <div className="absolute top-0 right-0 p-8 flex flex-col items-end gap-1 opacity-20 select-none">
                     <span className="text-[8px] font-jet-mono text-white/40 uppercase tracking-widest">{plan.name}_ACCESS_NODE</span>
                     <div className="h-px w-24 bg-gradient-to-l from-white/40 to-transparent" />
                  </div>

                  {plan.recommended && (
                    <div className="absolute top-0 left-12 px-5 py-2 bg-p text-white text-[9px] font-black uppercase tracking-[0.3em] rounded-b-xl shadow-2xl font-syne">
                      Recommended_Unit
                    </div>
                  )}
                  ...
                  <div className="mb-12 relative z-10 pt-4">
                    ...
                    <h4 className="text-4xl font-syne font-black mb-4 tracking-tighter uppercase italic">{plan.name}</h4>
                    <div className="flex items-baseline gap-2 mb-8">
                      <span className="text-6xl font-syne font-black tracking-tighter text-white">
                        {billingCycle === 'monthly' ? plan.price.monthly : plan.price.yearly}
                      </span>
                      {plan.price.monthly !== 'Custom' && (
                        <span className="text-white/20 text-xs font-black uppercase tracking-[0.2em] font-syne italic">/ PULSE</span>
                      )}
                    </div>
                    <p className="text-white/40 text-lg leading-relaxed font-syne font-medium italic tracking-tight">
                      {plan.description}
                    </p>
                  </div>
                  ...
                  <div className="space-y-4 mb-12 flex-grow relative z-10">
                    {plan.features.map((feature) => (
                      <div key={feature} className="flex items-center gap-4 group/item">
                        ...
                        <span className="text-sm text-white/50 group-hover/item:text-white transition-colors font-syne font-medium italic tracking-tight">{feature}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="mt-auto relative z-10">
                    <Button 
                      className={cn(
                        'w-full h-20 rounded-[28px] font-syne font-black text-sm uppercase tracking-[0.3em] italic group transition-all duration-700 relative overflow-hidden shadow-2xl',
                        plan.recommended
                    ...
                    {plan.name === 'Starter' && (
                      <div className="flex items-center justify-center gap-2 mt-6">
                         ...
                         <p className="text-[10px] text-white/20 uppercase tracking-[0.15em] font-black font-syne">
        ...
        >
          ...
          <div className="flex flex-col md:flex-row items-center gap-12 relative z-10">
            ...
            <div className="text-center md:text-left">
              <h5 className="text-4xl font-syne font-black mb-4 tracking-tighter uppercase italic">Institutional Forge</h5>
              <p className="text-white/40 text-xl max-w-xl leading-relaxed italic font-syne font-medium tracking-tight">
          ...
          <Button variant="ghost" className="h-20 px-12 border-2 border-white/5 bg-white/[0.02] hover:bg-p hover:text-white rounded-[28px] font-syne font-black text-sm uppercase tracking-[0.4em] italic transition-all duration-500 z-10 shadow-2xl">
            Request_Deployment
          </Button>
