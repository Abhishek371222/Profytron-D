  ...
  return (
    ...
    >
      <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-white/40 font-medium">Node ID: {selectedNodeId.split('_')[0]}</p>
          </div>
        </div>
        <button 
          ...
          <div className="space-y-6">
            {Object.entries(selectedNode.data.params).map(([key, value]) => (
              <div key={key} className="space-y-2">
                <label className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] flex items-center justify-between">
                  {key}
      ...
      <div className="p-6 border-t border-white/5 space-y-4 bg-white/[0.02]">
        <Button 
          onClick={() => selectNode(null)}
          className="w-full bg-gradient-to-r from-indigo-600 to-indigo-500 h-12 rounded-xl text-xs font-black uppercase tracking-widest shadow-lg shadow-indigo-600/20"
        >
          Update Logic
        </Button>
        <Button 
          ...
          className="w-full text-rose-500 hover:text-rose-400 hover:bg-rose-500/10 h-10 text-[10px] items-center gap-2 translate-y-2"
        >
