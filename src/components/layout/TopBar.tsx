    ...
    <header className="h-20 w-full glass border-b border-border-default flex items-center justify-between px-8 sticky top-0 z-30">
      ...
      <div className="flex-1 max-w-xl">
        ...
        >
          ...
          <div className="ml-auto flex items-center gap-1.5 px-1.5 py-0.5 rounded border border-white/10 bg-white/5 text-[10px] font-mono">
          ...
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="h-12 gap-3 pl-2 pr-4 rounded-full bg-white/5 hover:bg-white/10 border border-border-faint transition-all">
              ...
              <div className="flex flex-col items-start">
                <span className="text-xs font-semibold leading-none">{user?.name || "Alexander Voss"}</span>
                <span className="text-[10px] text-p uppercase tracking-tighter font-bold">{user?.tier || "Institutional"}</span>
              </div>
