...
export function LandingNavbar() {
  ...
  useEffect(() => {
    ...
    // Initial check in case user refreshed while scrolled
    handleScroll();
        ...
        >
          ...
          <div className="hidden lg:flex items-center gap-8 bg-white/[0.03] border border-white/5 py-2 px-8 rounded-full backdrop-blur-3xl shadow-inner">
            {navLinks.map((link) => (
              <Magnetic key={link.name} strength={0.2}>
                <Link
                  href={link.href}
                  className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 hover:text-white transition-all duration-300 relative group/link"
                >
                  {link.name}
          ...
          <div className="hidden md:flex items-center gap-4">
            <Magnetic strength={0.1}>
              <Link href="/login">
                <Button variant="ghost" className="hover:bg-white/5 font-black text-[10px] tracking-[0.2em] uppercase text-white/50 hover:text-white">
                  Client Portal
                </Button>
              </Link>
            </Magnetic>
            <Magnetic strength={0.25}>
              <Link href="/register">
                <Button className="bg-primary hover:bg-primary/90 text-primary-foreground font-black text-xs px-8 h-12 rounded-[14px] group transition-all duration-500 shadow-[0_0_30px_rgba(99,102,241,0.3)] hover:shadow-[0_0_50px_rgba(99,102,241,0.6)] relative overflow-hidden uppercase tracking-widest">
          ...
          >
            ...
            <div className="flex flex-col gap-3 pt-6 border-t border-white/5">
              <Link href="/login" onClick={() => setMobileMenuOpen(false)}>
                <Button variant="outline" className="w-full h-14 text-sm font-black uppercase tracking-widest rounded-[14px] border-white/10 bg-white/5 text-white">
                  Client Portal
                </Button>
              </Link>
              <Link href="/register" onClick={() => setMobileMenuOpen(false)}>
                <Button className="w-full h-14 text-sm font-black uppercase tracking-widest rounded-[14px] bg-primary text-white shadow-lg shadow-primary/20">
                  Open Terminal
                </Button>
              </Link>
            </div>
