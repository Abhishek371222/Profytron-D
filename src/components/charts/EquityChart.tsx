  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-black/60 backdrop-blur-xl border border-white/10 p-4 rounded-2xl shadow-2xl">
        <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-1">{label} UTC</p>
        <div className="flex items-center gap-3">
          <p className="text-lg font-black text-white font-mono">${payload[0].value.toLocaleString()}</p>
          <span className="text-[10px] font-black text-green-500 bg-green-500/10 px-1.5 py-0.5 rounded-full">+1.2%</span>
        </div>
      </div>
    );
  }
