          ...
          >
            ...
            <div className="mt-12 space-y-4 text-center lg:text-left">
              <h2 className="text-4xl font-display font-bold text-white leading-tight">
                {type === 'login' ? (
                  <>Institutional Intelligence <br /> <span className="text-p font-accent italic font-normal">at your fingertips.</span></>
                ) : (
                  <>The Future of Trading <br /> <span className="text-s font-accent italic font-normal">starts with you.</span></>
                )}
              </h2>
          ...
          <div className="flex items-center gap-6">
            <div className="flex -space-x-3">
              {[1, 2, 3, 4, 5].map((i) => (
                <div 
                  key={i} 
                  className="w-10 h-10 rounded-full border-2 border-bg-card bg-bg-elevated flex items-center justify-center text-[10px] font-black shadow-2xl relative overflow-hidden group/avatar"
                >
              ...
              <div className="w-10 h-10 rounded-full border-2 border-bg-card bg-p flex items-center justify-center text-[10px] font-black text-white shadow-2xl">
                +12k
              </div>
            </div>
...
const TrustItem = ({ icon: Icon, label }: { icon: any; label: string }) => (
  <div className="flex items-center gap-3 text-[10px] uppercase tracking-[0.2em] font-black text-slate-500 hover:text-p transition-colors cursor-default">
