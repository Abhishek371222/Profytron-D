...
const PlanBadge = ({ plan }: { plan: Plan }) => {
  switch (plan) {
    case 'ELITE':
      return <span className="px-2 py-1 rounded text-xs font-bold tracking-wider bg-purple-500/10 text-purple-400 border border-amber-500/50">ELITE</span>;
    case 'PRO':
      return <span className="px-2 py-1 rounded text-xs font-bold tracking-wider bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">PRO</span>;
    case 'FREE':
    default:
      return <span className="px-2 py-1 rounded text-xs font-bold tracking-wider bg-slate-500/10 text-slate-400 border border-slate-500/20">FREE</span>;
  }
};
...
const KYCBadge = ({ kyc }: { kyc: KYC }) => {
  switch (kyc) {
    case 'VERIFIED':
      return <span className="px-2 py-1 rounded text-xs font-bold tracking-wider bg-green-500/10 text-green-500 border border-green-500/20">VERIFIED</span>;
    case 'PENDING':
      return <span className="px-2 py-1 rounded text-xs font-bold tracking-wider bg-amber-500/10 text-amber-500 border border-amber-500/20">PENDING</span>;
    case 'REJECTED':
      return <span className="px-2 py-1 rounded text-xs font-bold tracking-wider bg-red-500/10 text-red-500 border border-red-500/20">REJECTED</span>;
    case 'NOT_STARTED':
    default:
      return <span className="px-2 py-1 rounded text-xs font-bold tracking-wider bg-slate-500/10 text-slate-400 border border-slate-500/20">NOT STARTED</span>;
  }
};
