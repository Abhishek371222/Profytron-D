      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-black text-p uppercase tracking-[0.5em] font-jet-mono">Secure Key Infrastructure</span>
          </div>
          <h2 className="text-4xl font-black text-white uppercase tracking-tighter font-syne">API Terminal</h2>
          <p className="text-xs text-white/30 font-black uppercase tracking-[0.2em] max-w-lg">
            Manage cryptographic access tokens and monitor live API telemetry in real-time.
          </p>
        </div>
        <button
          onClick={() => setIsCreating(true)}
          className="flex items-center gap-3 h-14 px-8 rounded-[18px] bg-white text-black hover:bg-white/90 font-black text-[11px] uppercase tracking-[0.2em] shadow-[0_0_30px_rgba(255,255,255,0.1)] transition-all group"
        >
          ...
          >
            ...
            <div className="flex items-start gap-4 relative z-10">
              ...
              <div className="space-y-1.5">
                <p className="text-xs font-black text-emerald-400 uppercase tracking-widest">Key Generated — Copy Now. This cannot be viewed again.</p>
            ...
            <button
              ...
              className="shrink-0 flex items-center gap-2 h-10 px-5 rounded-[12px] bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/30 transition-all text-[11px] font-black uppercase tracking-widest relative z-10"
            >
      ...
      <div className="flex items-center gap-2 p-1.5 bg-white/[0.02] border border-white/5 rounded-[18px] w-fit">
        {(['keys', 'terminal'] as const).map(tab => (
          <button
            ...
            className={cn(
              "relative px-6 py-2.5 rounded-[14px] text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-300",
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
                  ...
                  <div className="flex items-start gap-5 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 space-y-2 flex-1">
                      <div className="flex items-center gap-3 flex-wrap">
                        <span className="text-sm font-black text-white uppercase tracking-tight">{key.name}</span>
                        <div className={cn(
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        {key.permissions.map(p => (
                          <span key={p} className={cn("px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border", PERMISSION_COLORS[p])}>
                            {p}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="grid grid-cols-3 gap-6 lg:w-auto shrink-0">
                    <div className="flex flex-col items-center gap-1 text-center">
                      <span className="text-[9px] font-black text-white/20 uppercase tracking-widest">Requests</span>
                      <span className="text-sm font-black text-white font-jet-mono">{key.requests.toLocaleString()}</span>
                    </div>
                    <div className="flex flex-col items-center gap-1 text-center">
                      <span className="text-[9px] font-black text-white/20 uppercase tracking-widest">Last Used</span>
                      <span className="text-xs font-black text-white/60 font-jet-mono whitespace-nowrap">{key.lastUsed}</span>
                    </div>
                    <div className="flex flex-col items-center gap-1 text-center">
                      <span className="text-[9px] font-black text-white/20 uppercase tracking-widest">Rate Limit</span>
                      <span className="text-xs font-black text-cyan-400 font-jet-mono whitespace-nowrap">{key.rateLimit}</span>
                    </div>
                  </div>
            ...
            <div className="rounded-[28px] bg-[#030303] border border-white/10 overflow-hidden shadow-[0_30px_60px_rgba(0,0,0,0.8)]">
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-white/[0.01]">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2 text-[10px] font-black text-white/30 uppercase tracking-widest font-jet-mono">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest font-jet-mono">LIVE</span>
                </div>
              </div>
              ...
              <div className="p-6 space-y-2 font-jet-mono">
                {TERMINAL_LOGS.map((log, i) => (
                  ...
                  >
                    ...
                    <span className={cn("font-black uppercase shrink-0 w-14", getMethodColor(log.method))}>{log.method}</span>
                    <span className="text-white/50 flex-1 truncate group-hover/log:text-white/80 transition-colors">{log.endpoint}</span>
                    <span className={cn("font-black shrink-0 w-10 text-right", getStatusColor(log.status))}>{log.status}</span>
              ...
              <div className="flex items-center justify-between px-6 py-4 border-t border-white/5 bg-white/[0.01] flex-wrap gap-4">
                {[
                  ...
                  <div key={s.label} className="flex items-center gap-3">
                    ...
                    <span className={cn("text-xs font-black font-jet-mono", s.color)}>{s.val}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          >
            ...
            >
              ...
              <div className="relative z-10 space-y-2">
                ...
                <h3 className="text-2xl font-black font-syne text-white uppercase tracking-tighter">Create New API Key</h3>
              </div>

              <div className="relative z-10 space-y-6">
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-white/30 uppercase tracking-[0.3em]">Key Identity Label</label>
                  <input
                    value={newKeyName}
                    onChange={e => setNewKeyName(e.target.value)}
                    placeholder="e.g. Production Trading Node"
                    className="w-full h-14 bg-white/[0.03] border border-white/10 rounded-[16px] px-5 text-sm font-black text-white placeholder:text-white/20 outline-none focus:border-p/50 transition-all font-jet-mono"
                  />
                </div>

                <div className="space-y-3">
                  <label className="text-[10px] font-black text-white/30 uppercase tracking-[0.3em]">Permission Scope</label>
                  <div className="flex flex-wrap gap-3">
                    {['READ', 'TRADE', 'ANALYTICS', 'ADMIN'].map(p => (
                      <button
                        ...
                        className={cn(
                          "px-4 py-2 rounded-[10px] text-[10px] font-black uppercase tracking-widest border transition-all",
                ...
                <div className="p-4 rounded-[14px] bg-amber-500/5 border border-amber-500/20 flex items-start gap-3">
                  ...
                  <p className="text-[10px] text-white/40 font-black uppercase tracking-wider leading-relaxed">
              ...
              <div className="flex gap-4 relative z-10">
                <Button
                  ...
                  className="flex-1 h-12 bg-white/5 border border-white/10 hover:bg-white/10 text-white text-[11px] font-black uppercase tracking-widest rounded-[14px]"
                >
                  Cancel
                </Button>
                <Button
                  ...
                  className="flex-[2] h-12 bg-white text-black hover:bg-white/90 text-[11px] font-black uppercase tracking-[0.2em] rounded-[14px] shadow-[0_0_20px_rgba(255,255,255,0.1)] disabled:opacity-30"
                >
                  Generate Key
                </Button>
              </div>
