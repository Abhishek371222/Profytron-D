  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-4">
        <p className="text-[10px] font-bold text-white/40 uppercase tracking-[2px]">{label}</p>
  ...
  return (
    <div className="card p-6 h-full flex flex-col justify-between">
      <div className="flex justify-between items-center mb-6">
        ...
        <Button variant="outline" size="sm" className="h-7 px-3 border-red-500/50 text-red-500 hover:bg-red-500 hover:text-white text-[10px] font-black uppercase">
          Kill Switch
        </Button>
      </div>
      ...
      <div className="relative flex items-center justify-center py-4">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-2xl font-black text-white font-mono">{Math.round(percentage)}%</span>
          <span className="text-[10px] text-white/40 font-bold uppercase tracking-widest">of limit</span>
        </div>
      </div>
      ...
      <div className="space-y-4 mt-6">
        <div>
          <div className="flex justify-between items-center mb-2">
            <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Daily Loss Cap</span>
            <span className="text-[10px] font-mono text-white/60">n1,620 / n10,000</span>
          </div>
          <Progress value={16.2} className="h-1" />
        </div>
        <div>
          <div className="flex justify-between items-center mb-2">
            <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Max Drawdown</span>
            <span className="text-[10px] font-mono text-white/60">8.4% / 15.0%</span>
          </div>
      ...
      {percentage > 70 && (
        <div className="mt-4 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center gap-2 animate-pulse">
          ...
          <p className="text-[10px] text-amber-500 font-bold uppercase tracking-tight">Warning: High risk exposure</p>
        </div>
      )}
    </div>
  );
...
function TradeRow({ trade, onExplain }: { trade: any; onExplain: (t: any) => void }) {
  return (
    ...
    >
      ...
      <div>
        <span className={cn(
          "text-[10px] font-black px-2 py-0.5 rounded-full border tracking-widest",
      ...
      <div className="flex justify-end">
        <Button variant="ghost" size="sm" className="h-8 px-3 text-p hover:text-p hover:bg-p/10 gap-2 font-black uppercase tracking-widest">
    ...
    <div className={cn("space-y-6", !mounted && "animate-pulse")} suppressHydrationWarning>
      ...
      <div className="card p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            <h2 className="text-xl font-bold font-display text-white italic tracking-tight">Institutional Performance Vector</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
          {[
            ...
            <div key={i} className="px-4 py-2 bg-white/5 border border-white/10 rounded-xl">
              ...
              <p className="text-sm font-black text-white font-mono mt-0.5">{stat.value}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 card p-6 overflow-hidden">
          <div className="flex justify-between items-center mb-6">
            ...
            <Link href="/strategies" className="text-[10px] font-black text-white/40 hover:text-p transition-colors uppercase tracking-widest">
              Manage All →
            </Link>
          </div>
              ...
              >
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-baseline">
                    <span className="text-[10px] font-bold text-white/40 uppercase">Daily P&L</span>
                    <span className="text-lg font-black text-green-500 font-mono">+$842</span>
                  </div>
                  
                  <div>
                    <div className="flex justify-between mb-1.5">
                      <span className="text-[10px] font-bold text-white/40 uppercase text-[10px]">Win Rate</span>
                      <span className="text-[10px] font-bold text-white font-mono">{strat.winRate}%</span>
                    </div>
      ...
      <div className="card overflow-hidden">
        <div className="p-6 border-b border-white/5 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
             <Button variant="ghost" size="sm" className="h-8 px-3 text-white/40 hover:text-white uppercase text-[10px] font-black">Export CSV</Button>
             <Link href="/history" className="text-[10px] font-black text-p uppercase tracking-widest">View All →</Link>
          </div>
        </div>
          ...
          <div className="min-w-[1000px]">
            <div className="grid grid-cols-7 px-6 py-3 border-b border-white/5 bg-white/[0.01]">
              {['Symbol', 'Type', 'Volume', 'Entry', 'Current P&L', 'Duration', 'Action'].map(head => (
                <span key={head} className="text-[10px] font-black text-white/30 uppercase tracking-[2px]">
                  {head}
                </span>
              ))}
            </div>
          ...
          <>
            ...
            >
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-lg font-bold text-white uppercase tracking-widest flex items-center gap-3 italic">
              ...
              <div className="space-y-8 overflow-y-auto flex-1 pr-2 custom-scrollbar">
                ...
                <div className="p-6 rounded-3xl bg-white/5 border border-white/10 relative overflow-hidden">
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      <h3 className="text-2xl font-black text-white font-mono">{selectedTrade.asset}</h3>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
                    <div>
                      <p className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Open Time</p>
                      <p className="text-xs font-mono text-white/70 mt-1">22:14:05 UTC</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Strategy</p>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <h4 className="text-[10px] font-bold text-white/40 uppercase tracking-widest">AI Confidence Score</h4>
                    <span className="text-lg font-black text-green-500 font-mono">74/100</span>
                  </div>
                  <Progress value={74} className="h-2" />
                  <p className="text-xs text-white/60 font-medium italic">"High conviction signal based on multi-timeframe divergence."</p>
                </div>

                {/* Reasoning List */}
                <div className="space-y-4">
                  <h4 className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Execution Logic</h4>
                ...
                <div className="p-4 rounded-2xl bg-amber-500/5 border border-amber-500/10 space-y-3">
                  ...
                  <ul className="space-y-2">
                   <li className="text-[11px] text-amber-500/70 font-bold leading-normal">• Volatility expansion expected in 14m (Tier 1 Data)</li>
                   <li className="text-[11px] text-amber-500/70 font-bold leading-normal">• Counter-trend momentum strengthening locally</li>
                  </ul>
                </div>
                ...
                <div className="flex flex-wrap gap-2 pt-4">
                  {[
                    ...
                    <div key={i} className="px-3 py-1.5 rounded-full bg-white/5 border border-white/10 flex items-center gap-2">
                      <span className="text-[9px] font-bold text-white/30 uppercase">{level.l}</span>
                      <span className="text-[10px] font-mono font-bold text-white">{level.v}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="pt-8 mt-auto">
                <Button className="w-full h-12 bg-white text-black hover:bg-white/90 rounded-2xl font-black uppercase tracking-widest">
