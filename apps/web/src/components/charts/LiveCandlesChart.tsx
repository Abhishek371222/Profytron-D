...
export function LiveCandlesChart({ data: _data, rangeLabel }: LiveCandlesChartProps) {
	...
	const isSynthetic = marketQuery.data?.source === 'profytron-market-v1';
	const sourceLabel = hasApiData ? (isSynthetic ? 'Synthetic' : 'Live API') : 'Unavailable';
	...
	return (
		<div className="relative h-full w-full min-h-[360px] rounded-2xl border border-white/8 bg-black/20">
			<div className="absolute left-3 top-3 z-10 flex items-center gap-2 rounded-xl border border-white/10 bg-black/45 px-3 py-2 backdrop-blur-sm">
				<span className={cn('text-[10px] font-semibold uppercase tracking-[0.24em]', isSynthetic ? 'text-amber-300/70' : hasApiData ? 'text-emerald-300/70' : 'text-white/40')}>{sourceLabel}</span>
			...
			{isSynthetic && (
				<div className="pointer-events-none absolute inset-x-4 bottom-3 z-10 rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-amber-300/60">
					Synthetic candles — add Twelve Data key for live feed
				</div>
			)}
			{marketQuery.isError && !hasApiData && (
				<div className="pointer-events-none absolute inset-x-4 bottom-3 z-10 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs font-medium text-red-200">
					Market feed unavailable. Check API connection.
				</div>
			)}
		</div>
	);
