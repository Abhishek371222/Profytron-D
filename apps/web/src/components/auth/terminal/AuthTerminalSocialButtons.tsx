...
export function AuthTerminalSocialButtons({ onGoogle, onGithub, itemVariants }: Props) {
  const [mounted, setMounted] = React.useState(false);
  React.useEffect(() => setMounted(true), []);

  if (!mounted) {
    return (
      <div className="at-social-stack" aria-hidden>
        <div className="at-social-btn at-social-btn--ghost" />
        <div className="at-social-btn at-social-btn--ghost" />
      </div>
    );
  }
  ...
  return (
    <div className="at-social-stack">
      <Btn
        type="button"
        {...(itemVariants ? { variants: itemVariants } : {})}
        onClick={onGoogle}
        className="at-social-btn"
      >
        <GoogleLogo />
