...
const GlobePoints = () => {
 ...
 const count = 400; // Increased for fidelity
...
const DataArc = ({ start, end, progress = 0 }: { start: THREE.Vector3; end: THREE.Vector3; progress?: number }) => {
 const curve = useMemo(() => {
 const mid = new THREE.Vector3()
 .addVectors(start, end)
 .multiplyScalar(0.5)
 .normalize()
 .multiplyScalar(2.6); // Higher arc
...
export function ThreeGlobe() {
 return (
 <div className="w-full h-full relative min-h-[500px] flex items-center justify-center cursor-crosshair">
 <Canvas
