...
import { JsonLd, FaqItem } from "@/components/seo/JsonLd";
...
function FaqAccordionRow({ item, index }: { item: FaqItem; index: number }) {
    ...
    <motion.div
      ...
      className={cn(
        ...
        open
          ...
          : "border-white/[0.07] bg-muted/2 hover:border-white/[0.14] hover:bg-muted/35"
      )}
    >
      <button
        onClick={() => setOpen((v) => !v)}
      ...
      >
        ...
        <span
          className={cn(
            ...
            open ? "bg-primary/20 rotate-180" : "bg-foreground/5"
          )}
        >
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <div className="px-6 pb-5 text-sm text-foreground/50 leading-relaxed border-t border-white/[0.05] pt-4">
              {item.answer}
            </div>
          </motion.div>
        )}
  ...
  return (
    <section className="relative py-24 px-4" id="faq">
      ...
      <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
        <div className="w-[600px] h-[400px] bg-primary/5 blur-[120px] rounded-full" />
      </div>

      <div className="relative max-w-6xl mx-auto">
        {/* Header */}
        <div className="text-center mb-16">
          <AnimatedGradientText className="text-foreground/60 text-xs mb-4">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold font-serif text-foreground mt-3 mb-4">
          ...
          <p className="text-foreground/40 text-base max-w-xl mx-auto">
            ...
            <a href="mailto:support@profytron.com" className="text-primary hover:text-primary transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          <div className="space-y-3">
            {left.map((item, i) => (
              <FaqAccordionRow key={i} item={item} index={i} />
            ))}
          </div>
          <div className="space-y-3">
            {right.map((item, i) => (
              <FaqAccordionRow key={i + half} item={item} index={i + half} />
            ))}
          </div>
        </div>
      </div>
