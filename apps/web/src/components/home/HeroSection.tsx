...
export function HeroSection() {
  ...
  return (
    ...
    >
      ...
      {/* Mobile ambient gradient — replaces canvas on small screens */}
      <div
        className="absolute inset-0 z-0 pointer-events-none md:hidden"
        style={{
          background:
            "radial-gradient(ellipse 80% 60% at 70% 40%, rgba(139,92,246,0.15) 0%, transparent 55%), " +
            "radial-gradient(ellipse 60% 50% at 20% 70%, rgba(6,182,212,0.10) 0%, transparent 55%), " +
            "radial-gradient(ellipse 50% 40% at 50% 10%, rgba(99,102,241,0.10) 0%, transparent 60%)",
        }}
      />
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center gap-12 lg:gap-0">
          ...
          <div className="flex-1 max-w-[600px]">
            ...
            <motion.p
              ...
              className="text-[0.95rem] sm:text-[1.1rem] leading-relaxed mb-8 sm:mb-10 max-w-[490px]"
            ...
            {/* Mobile product preview — only visible below lg (where portfolio card is hidden) */}
            <motion.div
              initial={{ opacity: 0, y: 12 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.82, duration: 0.5 }}
              className="lg:hidden grid grid-cols-3 gap-2 mb-8"
            >
              {[
                { label: "Win Rate", val: "73.4%", color: "#34d399" },
                { label: "Execution", val: "< 12ms", color: "#22d3ee" },
                { label: "Traders", val: "12k+", color: "#a78bfa" },
              ].map((m) => (
                <div
                  key={m.label}
                ...
                >
                  ...
                  <span className="text-[10px] uppercase tracking-wider mt-0.5" style={{ color: "rgba(255,255,255,0.40)" }}>
                    {m.label}
                  </span>
                </div>
              ))}
            ...
            <motion.div
              ...
              className="flex flex-wrap items-center gap-5 sm:gap-10 pt-6 sm:pt-8"
