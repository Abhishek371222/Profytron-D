...
"w-[240px] bg-[#11111a]/80 backdrop-blur-3xl border rounded-3xl overflow-hidden transition-all duration-500 select-none",
 ...
 <span className="text-xs font-semibold text-white/30 uppercase tracking-[0.2em]">{meta.label}</span>
 ...
 <span className="text-xs font-bold text-white/30 uppercase tracking-widest">{key}</span>
 <div className="px-2 py-0.5 rounded-lg bg-white/5 border border-white/5 group-hover/item:border-white/20 transition-colors">
 <span className="text-xs font-semibold text-p font-mono">
  {typeof value === 'boolean' ? (value ? 'YES' : 'NO') : (value as any)}
 </span>
 </div>
 </div>
 ))}
 ...
 <span className="text-xs font-bold uppercase tracking-widest text-center leading-tight">Awaiting Logic Data</span>
 </div>
 )}
 </div>
