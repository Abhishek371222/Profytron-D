...
export class VpsController {
  ...
  @ApiResponse({ status: 200, description: 'OK' })
  startVps(@Param('id') id: string, @Req() req: any) {
    return this.vpsService.startVps(id, req.user.id);
  }

  @Post(':id/stop')
  @ApiOperation({ summary: 'Stop a VPS instance' })
  @ApiResponse({ status: 200, description: 'OK' })
  stopVps(@Param('id') id: string, @Req() req: any) {
    return this.vpsService.stopVps(id, req.user.id);
  }

  @Delete(':id')
  @ApiOperation({ summary: 'Delete a VPS instance' })
  @ApiResponse({ status: 200, description: 'OK' })
  deleteVps(@Param('id') id: string, @Req() req: any) {
    return this.vpsService.deleteVps(id, req.user.id);
  }
